...
async def process_video(client, query):
    ...
    try:
        ...
        await msg.edit_text("🔄 Encoding 720p...")
        ...
        process.wait()
        await msg.edit_text("📤 Uploading all qualities...")
        ...
        for out_file, res in files:
            if os.path.exists(out_file) and os.path.getsize(out_file) > 50000:
                ...
                await client.send_video(chat_id=query.message.chat.id, video=out_file, duration=int(duration), thumb=data.get("thumb"), caption=f"✅ **{res} Done!**\n📂 `{base_name}`\n📊 Size: {round(final_size, 2)} MB", supports_streaming=True, progress=progress, progress_args=(msg, f"📤 Uploading {res}..."))
        await msg.edit_text("✅ All 3 qualities (720p, 480p, 360p) uploaded!")
