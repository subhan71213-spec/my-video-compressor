...
# API Credentials - Environment Variables से लें
            ...
            if os.path.exists("ffmpeg_log.txt"):
                with open("ffmpeg_log.txt", "r") as f:
                    ...
                    if times:
                        ...
                        if percent >= last_percent + 5: # 5% गैप पर अपडेट
...
async def start(client, message):
    await message.reply_text("👋 Hi Shakeel!\nSend a video/document to start compressing. 🚀")
...
async def handle_video(client, message):
    ...
    await msg.edit_text("✅ Downloaded!\n\n📂 Now send the **New File Name** (with .mp4 extension):")
...
async def get_thumb(client, message):
    ...
    # Dynamic buttons based on file size
    if size >= 1800:
...
async def process_video(client, query):
    ...
    crf = 23 if target_mb >= 700 else 27
    preset = "veryfast"

    out = f"compressed_{uid}.mp4"
    msg = await query.message.edit_text(f"⚙️ Initializing compression to {target_mb}MB...")
    ...
    try:
        ...
        # 🔥 FIXED COMMAND: No multiline string issues
        cmd = (
            f'ffmpeg -i "{data["path"]}" -map 0 -c:v libx264 -preset {preset} '
            f'-crf {crf} -maxrate {bitrate}k -bufsize {bitrate*2}k -movflags +faststart '
            f'-c:a copy -c:s copy -threads 0 "{out}" -y > ffmpeg_log.txt 2>&1'
        )
        ...
        if not os.path.exists(out):
            await msg.edit_text("❌ FFmpeg Error: Output file not generated.")
            return
